...
const CreateAppointment = () => {
  ...
  const createNewslotes = (event) => {
    ...
    fetch("http://localhost:8080/timetable/addSlot", requestOptions)
      .then((response) => {
        console.log(response.status);
        if (response.ok) setMsg("Appointment Created");
        else {
          setMsg("Slot is not Created");
        }
      })
      .catch((errr) => {});
  };
  ...
  return (
    <>
      <div className="container-sm  p-4  w-75  m-auto shadow  mb-5 mt-5 rounded">
        <h3 className="mb-4 text-center">Create Slots for Appointment</h3>

        <div className="container-sm    w-75 ">
          <form
            className="formm bg-white m-auto shadow pt-4 border border-warning border-3 mb-5 mt-5 rounded"
            onSubmit={createNewslotes}
          >
            ...
            <div className="row w-100 gx-5 m-3 ">
              ...
              <div className="form-group col-lg-4 ">
                <label className="form-label" for="SlotTime">
                  SlotTime
                </label>
                <select
                  className=" border-2 form-control "
                  name="SlotTime"
                  id="SlotTime"
                  onClick={(e) => setSlotDuration(e.target.value)}
                >
            ...
            <div className="row w-100 m-3 gx-5">
              <div className="form-group   col-lg-4">
                <label className="form-label" for="BreakStartTime">
                  Break Start Time{" "}
                </label>
                <select
                  ...
                  name="BreakStartTime"
                  id="BreakStartTime"
                  onClick={(e) => setBreakStartTime(e.target.value)}
                >
                  <option value="11:00:00">11 am</option>
                  <option value="12:00:00">12 pm</option>
                  <option value="13:00:00">1 pm</option>
              ...
              <div className="form-group  col-lg-4">
                <label className="form-label" for="BreakEndTime">
                  Break End Time
                </label>
                <select
                  className=" border-2 form-control"
                  name="BreakEndTime"
                  id="BreakEndTime"
                  onClick={(e) => setBreakendTime(e.target.value)}
                >
                  <option value="12:00:00">12 pm</option>
                  <option value="13:00:00">1 pm</option>
                  <option value="14:00:00">2 pm</option>
